function [rxnList, stoichiometries] = findRxnsFromMetss(model, mets)
...
if nargout < 2
    rxnList = model.rxns(sum(abs(metStoich),2) > 0);
    return
else
    %Initialize the outputs.
    rxnList = cell(numel(mets),1);
    stoichiometries = cell(numel(mets,1));
    num_Rxnlist = cell(numel(mets,1)); 
    ...
    for i = 1:numel(metInd)
        ...
        relStoichiometries{i} = metStoich(relpos,i);
